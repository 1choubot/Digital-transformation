<template>
  <section v-if="form" ref="editorRoot" class="online-form-editor" aria-label="在线表单动作区">
    ...
    <GeneratedFormFileCard :generated-file="generatedFile" :pending="downloadPending" :button-text="downloadButtonText"
    ...
    <template v-if="showFormContent">
      ...
      <el-form class="online-form-editor__form" :model="formData" @submit.prevent="handleSubmit">
        <section v-if="isInitiationNotice" class="online-form-section initiation-notice-section">
          <div class="initiation-notice-table-wrap">
            <table class="initiation-notice-table">
              ...
              <tbody>
                <tr>
                  <td data-field-key="projectCode"
                    :class="{ 'online-form-field--invalid': isFieldInvalid('projectCode') }">
                    <el-input :model-value="formData.projectCode" :readonly="projectCodeField.readOnly"
                      :disabled="isOnlineFormFieldDisabled(projectCodeField)" aria-label="项目编号"
            ...
            <div class="requirement-template-table-wrap">
              <table class="requirement-template-table">
                ...
                <tbody>
                    ...
                    <td colspan="2">
                      <div class="requirement-template-metric">
                        ...
                        <span data-field-key="workingTemperatureMin"
                          :class="getFieldClass(getFormField('workingTemperatureMin'))"><el-input
                            :model-value="formData.workingTemperatureMin"
                            :disabled="isOnlineFormFieldDisabled(getFormField('workingTemperatureMin'))"
                            aria-label="工作温度最小值"
                            @update:model-value="$emit('update-field', { key: 'workingTemperatureMin', value: $event })" /></span>
                        <span>）℃～（</span>
                        <span data-field-key="workingTemperatureMax"
                          :class="getFieldClass(getFormField('workingTemperatureMax'))"><el-input
                            :model-value="formData.workingTemperatureMax"
                            :disabled="isOnlineFormFieldDisabled(getFormField('workingTemperatureMax'))"
                            aria-label="工作温度最大值"
                            @update:model-value="$emit('update-field', { key: 'workingTemperatureMax', value: $event })" /></span>
                        <span>）℃</span>
                      </div>
                    </td>
                    <td colspan="2">
                      <div class="requirement-template-metric">
                        ...
                        <span data-field-key="storageTemperatureMin"
                          :class="getFieldClass(getFormField('storageTemperatureMin'))"><el-input
                            :model-value="formData.storageTemperatureMin"
                            :disabled="isOnlineFormFieldDisabled(getFormField('storageTemperatureMin'))"
                            aria-label="储存温度最小值"
                            @update:model-value="$emit('update-field', { key: 'storageTemperatureMin', value: $event })" /></span>
                        <span>）℃～（</span>
                        <span data-field-key="storageTemperatureMax"
                          :class="getFieldClass(getFormField('storageTemperatureMax'))"><el-input
                            :model-value="formData.storageTemperatureMax"
                            :disabled="isOnlineFormFieldDisabled(getFormField('storageTemperatureMax'))"
                            aria-label="储存温度最大值"
                            @update:model-value="$emit('update-field', { key: 'storageTemperatureMax', value: $event })" /></span>
                        <span>）℃</span>
                      </div>
                    </td>
                  </tr>
                  <tr>
                    <td colspan="2">
                      <div class="requirement-template-metric"><span>工作湿度：（</span><span
                          data-field-key="workingHumidityMin"
                          :class="getFieldClass(getFormField('workingHumidityMin'))"><el-input
                            :model-value="formData.workingHumidityMin"
                            :disabled="isOnlineFormFieldDisabled(getFormField('workingHumidityMin'))"
                            aria-label="工作湿度最小值"
                            @update:model-value="$emit('update-field', { key: 'workingHumidityMin', value: $event })" /></span><span>）%～（</span><span
                          data-field-key="workingHumidityMax"
                          :class="getFieldClass(getFormField('workingHumidityMax'))"><el-input
                            :model-value="formData.workingHumidityMax"
                            :disabled="isOnlineFormFieldDisabled(getFormField('workingHumidityMax'))"
                            aria-label="工作湿度最大值"
                            @update:model-value="$emit('update-field', { key: 'workingHumidityMax', value: $event })" /></span><span>）%</span>
                      </div>
                    </td>
                    <td colspan="2">
                      <div class="requirement-template-metric"><span>储存湿度：（</span><span
                          data-field-key="storageHumidityMin"
                          :class="getFieldClass(getFormField('storageHumidityMin'))"><el-input
                            :model-value="formData.storageHumidityMin"
                            :disabled="isOnlineFormFieldDisabled(getFormField('storageHumidityMin'))"
                            aria-label="储存湿度最小值"
                            @update:model-value="$emit('update-field', { key: 'storageHumidityMin', value: $event })" /></span><span>）%～（</span><span
                          data-field-key="storageHumidityMax"
                          :class="getFieldClass(getFormField('storageHumidityMax'))"><el-input
                            :model-value="formData.storageHumidityMax"
                            :disabled="isOnlineFormFieldDisabled(getFormField('storageHumidityMax'))"
                            aria-label="储存湿度最大值"
                            @update:model-value="$emit('update-field', { key: 'storageHumidityMax', value: $event })" /></span><span>）%</span>
                      </div>
                    </td>
                  </tr>
                  <tr>
                    <td colspan="2">
                      <div class="requirement-template-metric"><span>噪音：≤（</span><span data-field-key="noiseLimitValue"
                          :class="getFieldClass(getFormField('noiseLimitValue'))"><el-input
                            :model-value="formData.noiseLimitValue"
                            :disabled="isOnlineFormFieldDisabled(getFormField('noiseLimitValue'))" aria-label="噪音上限值"
                            @update:model-value="$emit('update-field', { key: 'noiseLimitValue', value: $event })" /></span><span>）dB</span>
                      </div>
                    </td>
                    <td colspan="2">
                      <div class="requirement-template-metric"><span>IP 防护等级：IP（</span><span
                          data-field-key="ipProtectionLevel"
                          :class="getFieldClass(getFormField('ipProtectionLevel'))"><el-input
                            :model-value="formData.ipProtectionLevel"
                            :disabled="isOnlineFormFieldDisabled(getFormField('ipProtectionLevel'))"
                            aria-label="IP 防护等级"
                            @update:model-value="$emit('update-field', { key: 'ipProtectionLevel', value: $event })" /></span><span>）</span>
                      </div>
                    </td>
                  </tr>
                  <tr>
                    <td colspan="2">
                      <div class="requirement-template-metric"><span>防腐等级：（</span><span
                          data-field-key="antiCorrosionGrade"
                          :class="getFieldClass(getFormField('antiCorrosionGrade'))"><el-input
                            :model-value="formData.antiCorrosionGrade"
                            :disabled="isOnlineFormFieldDisabled(getFormField('antiCorrosionGrade'))" aria-label="防腐等级"
                            @update:model-value="$emit('update-field', { key: 'antiCorrosionGrade', value: $event })" /></span><span>）</span>
                      </div>
                    </td>
                    <td colspan="2">
                      <div class="requirement-template-metric"><span>海拔高度：≤（</span><span
                          data-field-key="altitudeLimitValue"
                          :class="getFieldClass(getFormField('altitudeLimitValue'))"><el-input
                            :model-value="formData.altitudeLimitValue"
                            :disabled="isOnlineFormFieldDisabled(getFormField('altitudeLimitValue'))"
                            aria-label="海拔高度上限值"
                            @update:model-value="$emit('update-field', { key: 'altitudeLimitValue', value: $event })" /></span><span>）m</span>
                      </div>
                    </td>
                  </tr>
                  <tr>
                    <td colspan="4" data-field-key="explosionProofRequirement"
                      :class="getFieldClass(getFormField('explosionProofRequirement'))">
                      <div class="requirement-template-metric requirement-template-metric--wide">
                        <span>防爆要求：（</span>
                        <el-input :model-value="formData.explosionProofRequirement"
                          :disabled="isOnlineFormFieldDisabled(getFormField('explosionProofRequirement'))"
                          aria-label="防爆要求"
                          @update:model-value="$emit('update-field', { key: 'explosionProofRequirement', value: $event })" />
                        <span>）</span>
                      </div>
                    </td>
                  </tr>
                ...
                <template v-else v-for="field in [item.field]" :key="field.key">
                  <label :data-field-key="field.key" :class="getFieldClass(field)">
                    ...
                    <el-select v-else-if="field.type === 'score'" :model-value="formData[field.key]"
                      ...
                      <el-option v-for="score in scoreOptions" :key="score" :label="String(score)"
                        :value="String(score)" />
                    </el-select>
                    <el-input v-else-if="field.type === 'textarea'" type="textarea" :rows="3"
                      :model-value="formData[field.key]" :disabled="isOnlineFormFieldDisabled(field)"
                      @update:model-value="$emit('update-field', { key: field.key, value: $event })" />
                    <div v-else-if="field.type === 'image'" class="online-form-image-field">
                      ...
                      <el-upload class="online-form-image-field__upload" :show-file-list="false"
        ...
        <section v-for="section in form.schema?.scoringSections || []" :key="section.key"
          class="online-form-section online-form-score-section" :data-score-section-key="section.key">
          <button class="online-form-score-section__toggle" type="button"
            :aria-expanded="isScoringSectionExpanded(section)" :aria-controls="`score-section-${section.key}`"
          ...
          <div v-show="isScoringSectionExpanded(section)" :id="`score-section-${section.key}`"
                ...
                <article v-for="item in section.items || []" :key="item.key" :data-field-key="`${item.key}Score`"
                  ...
                  <dl class="online-form-score-card__template">
                    <div>
                      <dt>评价标准</dt>
                      <dd>
                        <span v-for="(line, lineIndex) in formatEvaluationStandardLines(item.evaluationStandard)"
                          :key="lineIndex">
                          {{ line }}
                        </span>
                      </dd>
        ...
        <section v-if="showReviewOpinions && displayedReviewOpinions.length" class="online-form-section">
          <div class="online-form-review-opinions"
            ...
            <article v-for="opinion in displayedReviewOpinions" :key="opinion.nodeKey"
