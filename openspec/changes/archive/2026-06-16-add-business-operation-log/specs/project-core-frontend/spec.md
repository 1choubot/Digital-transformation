...
前端 MUST 提供项目详情页，并 MUST 调用 `GET /api/projects/:projectId` 获取项目基础状态和创建人追溯字段，同时 MUST 展示该项目的阶段资料清单基础信息、状态、状态追溯字段、适用性、适用性追溯字段、手工状态操作、适用性操作、阶段资料齐套摘要、当前阶段手工推进入口和只读业务日志区域。
...
- **WHEN** 用户打开项目详情页
- **THEN** 除既有手工阶段推进、既有手工资料项适用性、既有只读阶段资料齐套摘要和只读业务日志区域外，页面不能展示在线表单填写入口、文件上传入口、文件下载入口、文件管理平台同步动作、复杂权限控制、角色权限控制、轻角色校验、管理层看板指标、项目类型模板自动标记不适用或补初始化按钮
