# Digital-transformation 项目阶段前端页面拆分与风格统一通用规则

> 适用工程：`digital-platform-web`。
>
> 适用范围：后续项目阶段节点页面新增、从通用阶段实现拆分节点页面，以及阶段页面体验统一。
>
> 样式治理依据：`docs/Vue样式集中治理记录_20260720.md`。
>
> 正式业务与架构契约：`openspec/specs/` 下 active specs；`openspec/changes/archive/` 仅用于追溯历史背景。

## 1. 总体原则

项目阶段前端应形成“项目工作区统一编排、节点页面拥有业务结构、阶段能力按稳定语义复用”的架构。页面拆分和视觉调整不得改变后端业务状态机、权限模型、API、DTO、数据库结构、节点推进结果或资料版本语义。

```text
阶段 API / DTO / permissions / blockingReasons / status
                         ↓
阶段 composable 与 API adapter
                         ↓
具名节点页面 + 稳定业务共享组件
                         ↓
nodePages.js + NodePageRouter.vue
                         ↓
ProjectDetailLayout 统一项目上下文、刷新和节点定位
```

前端不得根据角色名称、部门文案或本地状态重新推导权限或节点推进结果。现有字段不足时，应记录为独立 OpenSpec change，不得在页面中硬编码补齐。

## 2. 当前架构基线

- `src/pages/project-detail/ProjectDetailPage.vue` 是项目详情薄入口；
- `ProjectDetailLayout.vue` 负责项目、工作区、阶段、节点上下文及统一业务刷新；
- `src/config/nodePages.js` 负责节点代码到页面组件的注册；
- `src/pages/project-node/project-approval/NodePageRouter.vue` 负责向节点页面传递统一上下文；
- 立项节点位于 `src/pages/project-node/project-approval/`；
- 方案设计节点位于 `src/pages/project-node/solution-design/`；
- 合同签订节点位于 `src/pages/project-node/contract-signing/`；
- 阶段共享请求和状态协调位于 `src/composables/project-stage/<stage-name>/`；
- 跨节点的稳定业务组件位于 `src/components/project-workspace/<stage-name>/`，通用节点能力可位于 `src/components/node/` 和 `src/composables/node/`；
- `nodePageContext` 是项目详情层向节点页提供的统一上下文和动作边界，不是节点页自行扩展状态机的入口；
- 未注册节点由 `BlankProjectNodePage.vue` 兜底，不另建第二套路由协议。

方案设计阶段的旧总面板和迁移壳已退出运行路径。分析、评审、成本和报价/投标节点均已通过当前节点页面体系注册。

合同签订阶段的 `contract_preparation`、`contract_signing`、`advance_payment` 均已通过当前节点页面体系注册。三个节点共享 `ContractSigningNodeLayout` 和合同阶段业务组件，但继续保留独立页面入口、权限、文件槽、付款流程和节点动作；不得为了视觉统一重新合并为单一合同阶段总页面。

## 3. 页面拆分实施顺序

### 3.1 冻结业务基线

拆分前必须记录：

- 节点清单、顺序、角色和状态；
- 每个节点的可见、可编辑、可提交、可审批和可退回条件；
- API、payload、DTO、错误码、文件接口和 revision；
- 前置条件、阻塞原因、自动推进和退回重提规则；
- active OpenSpec 对该阶段的 MUST / MUST NOT 场景；
- 后端专项测试或可执行人工回归清单。

归档 change 只能解释历史决策，不能覆盖 active specs 和当前 API 行为。

### 3.2 建立节点注册

阶段节点页面统一放置在：

```text
src/pages/project-node/<stage-name>/<NodeName>Page.vue
```

节点统一注册到 `src/config/nodePages.js`，并通过现有 `NodePageRouter.vue` 接收项目上下文。不得在阶段内部新增平行路由、复制项目详情加载或自行刷新浏览器。

节点注册页可以是具名薄入口，将 props 和 `business-state-changed` 原样转交给稳定共享业务组件；但文件名、注册项和节点代码必须保持一一可追踪。不得为了减少文件数量而把多个业务不同的节点重新合并为按 `nodeKey` 分支的总面板。

### 3.3 抽取阶段共享能力

阶段 composable 可以负责：

- API 调用和 payload builder；
- DTO 到页面状态的无损适配；
- pending、错误、消息和并发请求失效；
- 上传、下载、生成文件和 Blob 保存；
- 操作成功后的统一刷新通知。

阶段 composable 不得负责 DOM、页面布局、本地权限推导、状态机改写或伪造乐观推进结果。

通用节点 UI 组件位于 `src/components/node/`；通用节点状态与行为逻辑位于 `src/composables/node/`。两者按展示层与逻辑层分工，不得重复承载同一职责。

共享业务组件负责同构节点的布局、表单和动作组合。共享条件必须来自稳定业务语义，例如相同的上传型节点、成本节点或评审模型；仅仅“页面长得相似”不足以合并。项目、工作区、导航和当前节点的全量刷新仍由 `ProjectDetailLayout.vue` 统一协调，阶段 composable 和节点页不得复制这套加载链路。

### 3.4 完成节点业务页面

节点页面负责：

- 本节点页面结构和操作编排；
- DTO 权限、阻塞原因和状态绑定；
- loading、empty、error、readonly、disabled；
- 调用阶段 composable；
- 上抛 `business-state-changed`。

禁止把所有节点表单和操作重新集中到一个按 `nodeKey` 大量分支的阶段总面板。

允许具名节点注册页复用稳定共享页面，但必须满足：

- 节点业务结构、数据和事件语义确实一致；
- 节点上下文仍能区分独立资料、revision、审批历史和生成文件；
- 共享页面不包含整个阶段其他节点的条件分支。

C15 内部评审和 C16 客户评审可以复用评审页面能力，但不得合并为一个资料项或不可区分的记录上下文。标准上传型节点可以复用上传页面，但各节点权限、上传槽和状态仍以后端 DTO 为准。

上传型节点优先使用统一的“上传内容 / 文件名 / 操作”表头和数据行结构。审批、退回等正式节点动作不得嵌入上传表格行，应使用统一审批组件放在节点主要业务内容或上传表格下方，与方案设计节点的动作层级一致。简单状态页应使用标题、状态、描述信息和动作的平铺层级，避免在项目工作区节点卡片内继续嵌套无必要的卡片容器。

### 3.5 合同签订阶段页面约束

- `ContractSigningNodeLayout` 是三个合同节点的唯一外层骨架，负责节点名称、节点状态及通用反馈；
- 通用反馈按 error → ended → returned → blocked 的优先顺序呈现，避免同一状态在页面内重复提示；
- 合同组件使用 `contract-signing-*`、`contract-upload-table-*` 等合同业务语义类，不依赖 `solution-*` 类名或方案设计业务组件；
- 方案设计页面可以作为视觉层级和响应式基准，但不得成为合同业务组件依赖；
- 合同阶段全部上传表格以方案设计上传表格为统一视觉和文本基准，采用“上传内容 / 文件名 / 操作”三列，只显示原始文件名；无文件时显示 `-`，不附加文件大小、上传人或上传时间；
- 准备节点按统一上传表格展示技术协议和销售合同，审批卡片位于表格下方；
- 签订节点按统一上传表格展示技术协议扫描件和销售合同扫描件，客户退回审批卡片位于相应节点业务区，不在表格操作列中实现；
- 预付款节点参照方案设计准备页使用平铺标题、状态、`el-descriptions`、提示和底部动作，不为支付状态、启动通知、付款处理重复增加卡片容器；
- `ContractSigningSection` 只用于确有独立语义的危险区或末尾主动作，不包裹上传表格和预付款普通信息区；
- 合同页面只调整前端结构和样式时，不得修改 API、DTO、权限、状态机、revision、确认流程、自动推进或 `business-state-changed` 协议。

## 4. 固定前端接口

节点页面继续接受路由约定的项目上下文 props，当前包括：

- `projectId`
- `authToken`
- `currentUser`
- `project`
- `workspace`
- `stage`
- `node`
- `nodeCode`
- `nodePageContext`

业务变化统一上抛：

```js
emit('business-state-changed', {
  source: '<stage-name>',
  nodeKey: props.nodeCode,
  changedDocumentIds: [],
  affectedNodeCodes: [],
  refreshCurrentDetail: false,
  clearCurrentDetail: false
});
```

要求：

- `source`、`nodeKey` 和 `changedDocumentIds` 是基础追踪字段；只有确有跨节点影响时才提供 `affectedNodeCodes`；
- `refreshCurrentDetail: true` 表示刷新后保留并重载当前在线表单详情，`clearCurrentDetail: true` 表示刷新前清空当前详情，二者不得同时为 `true`；
- 未设置的可选字段由项目详情层按 `false` 或空集合处理，不得由节点页赋予新的隐式语义；
- 保留调用方依赖的 payload 字段；
- pending 结束前阻止重复操作；
- 失败或用户取消时不得上抛成功刷新事件；
- 不在多个层级重复刷新；
- 自动推进后由项目详情层根据刷新前后导航状态重新定位当前阶段和节点；节点页不得自行猜测下一个节点。

## 5. 权限、状态和错误边界

- 按钮可见性和禁用状态以 DTO 的 `permissions`、在线表单权限、`blockingReasons` 和 `status` 为依据；
- 前端隐藏按钮不能代替后端鉴权；
- 前端不得用项目查看权限、部门、姓名或角色文案放宽操作权限；
- 操作失败必须展示可读业务错误，不得伪造成功状态；
- 生成文件失败不得展示节点已提交成功；
- 后端未自动推进时展示后端返回的缺项和阻塞原因，不在前端复制门禁。

## 6. 在线表单规则

### 6.1 保存与提交

- 保存草稿不强制完成全部提交必填项；
- 正式提交前校验当前用户负责范围内的必填字段；
- 未填字段使用红色边框和字段级提示，并汇总提示、滚动及聚焦首个缺项；
- 校验失败、接口失败或用户取消不得清空已填写数据；
- 日期和数字类型必须保持后端协议，不得因控件替换改变 payload 类型；
- 动态数组和结构化实施计划必须保留用户输入顺序和内容。

### 6.2 提交确认与自动推进

C05、C15、C16 点击“提交表单”时必须确认；用户取消不得调用接口。这些在线表单页面以“提交表单”作为节点推进入口，不得同时展示手动“提交节点”。

前端按后端结果分别展示：

- 表单、生成文件和节点自动提交均成功；
- 表单提交成功但仍有其他缺项；
- 生成文件失败；
- 节点门禁未满足。

前端不得维护 Excel 单元格、Word 表格或模板文本映射。

## 7. 文件和危险操作

- `el-upload` 只负责选择文件和 UI，网络请求继续使用现有 API 客户端和 Bearer token；
- 保留 MIME、大小、数量、revision、只读和后端下载文件名规则；
- 无权用户不得看到已缓存的敏感文件名或附件明细；
- 上传、删除或表单字段变化导致生成文件失效时，页面必须反映最新 DTO 状态；
- 退回、删除、结束项目、废弃资料等危险操作使用 `ElMessageBox.confirm`；
- 用户取消时不发送请求、不改变本地状态、不触发刷新事件。

### 7.1 上传表格统一规则

- 阶段上传表格统一采用带 `role="table"`、`role="row"`、`role="columnheader"` 和 `role="cell"` 的响应式网格结构；除非确有分页、排序等复杂需求，不因“表格”名称强制改用 `el-table`；
- 标准上传表格统一使用“上传内容 / 文件名 / 操作”三列表头；合同上传表格不得增加独立“状态”列；
- 文件列只显示后端返回的原始文件名，无文件时显示 `-`；不得在标准上传表格中附加文件大小、上传人或上传时间，长文件名省略显示并通过 `title` 保留完整内容；
- 合同上传表格的容器宽度、三列宽度、行高、内边距、边框、圆角、背景、分隔线、文件名文字颜色和响应式行为均以方案设计上传表格为基准；合同组件继续使用自身业务类名和 scoped 样式，不直接依赖 `solution-*` 类名或方案设计组件；
- 桌面端标准布局采用 `200px minmax(260px, 1fr) 264px` 三列，数据行最小高度为52px、表头最小高度为42px、行内边距为 `8px 14px`；窄屏改为 `minmax(100px, 0.8fr) minmax(0, 1.2fr)` 两列，隐藏操作列表头并让操作区独占下一行；
- 上传控件统一使用 `show-file-list="false"`、`auto-upload="true"` 和现有 `http-request` 事件适配，不改变 API 客户端、事件参数和文件校验；
- 上传按钮使用实心 primary，自然宽度，固定基础文案“上传/替换”；当前节点或文件槽进入后端定义的退回状态后统一显示“整改重传”；
- “整改重传”仅是展示文案，首次上传、替换、revision 和整改后的后端处理仍以原 API 与 DTO 为准；
- 下载按钮使用实心 primary，固定文案“下载”，仅在存在当前文件且 `canDownload` 为真时展示；
- 上传按钮仅由 `canUpload` 控制。方案设计独有的“无需上传/取消无需上传”不得扩展到没有相应 DTO、权限和 API 的阶段；
- 上传 pending key 使用 `upload:${slot.slotKey}`，下载 pending key 使用 `download:${slot.slotKey}`，pending 期间阻止重复操作；
- 操作列采用12px间距语义、左对齐、按钮自然宽度，不拉伸或等分；窄屏下操作区可占满整行，但按钮仍保持左对齐和自然宽度；
- 表格只承载文件操作，正式审批、审批退回、客户退回和节点完成动作放在表格下方的独立动作层。

### 7.2 统一审批组件

审批、审批退回、客户退回和结束项目类交互优先使用统一 `ApprovalActionCard`，不得在节点页面重复实现 textarea、意见必填、pending 锁定和按钮禁用规则。

- 审批组件应直接放在节点主要业务内容或上传表格下方，不嵌入文件数据行或操作列；
- 同一审批动作同时支持通过和退回时展示组合意见提示；仅支持退回时明确提示“退回原因必填”；
- `showComment` 默认开启；后端不保存审批意见的场景应显式关闭，避免制造无效输入；
- 需要先选择业务事实或分支的审批使用 selection 插槽和 `selectionRequired`，选择未完成时由组件统一禁用通过动作；
- 审批人选择业务事实、执行模式或后续流程分支时，固定且互斥的 2～4 个选项统一使用 `ApprovalBusinessSelection`，其内部采用 `el-radio-group` 和带 `border` 的 `el-radio`，所有选项必须同时可见；
- 业务选择标签统一展示字段名称、必填标识和“审批通过前必须选择”提示；每个选项占满所在网格单元且最小高度为40px，选中项使用 primary 浅色背景和边框；
- 业务选择在桌面端按选项数量等宽排列，640px以下统一改为单列；超过4个选项、选项动态加载或需要搜索时才使用 `el-select`；
- 统一业务选择不得改用 `el-radio-button`，不得因控件和布局统一改变 API payload 的真实值；页面继续通过 `selectionRequired` 和 `selectionComplete` 控制审批通过按钮；
- 页面只绑定 DTO 权限、评论模型、selection、pending key 和原业务方法，不在组件内重新推导角色、权限或状态；
- 完成签订、完成支付、申请总经理放行等属于普通业务动作，不因临近审批区就强行纳入审批组件；
- 业务要求保留二次确认的客户退回、结束项目等危险动作，确认仍由阶段 composable 负责；用户取消时不请求、不刷新；
- 统一组件只负责展示、输入校验和按钮状态，不修改审批 API、payload、权限、自动推进和成功提示。

## 8. Element Plus 和样式职责

普通业务控件统一使用 Element Plus：

| 场景 | 组件 |
|---|---|
| 操作按钮 | `el-button` |
| 文本、多行文本和数字 | `el-input`、`el-input-number` |
| 选择、单选和复选 | `el-select`、`el-radio`、`el-checkbox` |
| 日期 | `el-date-picker`，显式设置 `value-format` |
| 表单 | `el-form`、`el-form-item` |
| 表格和状态 | `el-table`、`el-tag` |
| 错误、加载和空状态 | `el-alert`、`v-loading`、`el-empty` |
| 确认和短反馈 | `ElMessageBox.confirm`、`ElMessage` |
| 上传 | 受控 `el-upload` + `http-request` |

流程树、节点导航、复杂业务表格和图片布局可保留原生结构，其中的标准输入和操作仍使用 Element Plus。

操作颜色保持稳定语义：

- 主流程和文件上传下载使用 primary；
- 待审批和例外路径使用 warning；
- 退回及破坏性动作使用 danger；
- 已完成状态使用 success。

样式职责：

- `styles.css` 负责设计变量、主题、App shell、共享布局原语、项目工作区和响应式边界；
- scoped 样式负责组件专属复杂业务结构；
- 阶段专属表格、分区和响应式规则优先保留在对应组件 scoped 样式，不向 `styles.css` 增加单阶段业务规则；
- 禁止同一职责同时存在于全局和局部样式；
- `:deep()` 必须受组件根类约束，并仅在组件公开能力不足时使用；
- Vue 局部样式禁止 `!important` 和硬编码品牌/状态色；
- 不得为了追求 scoped 为零而把单组件规则堆入 `styles.css`。

内容分区使用16px间距语义、浅边框和轻背景；避免重阴影、大块空白和在节点工作区卡片内反复嵌套卡片。新增 scoped 文件必须进入样式治理白名单，除非明确调整治理基线，不得提高 `styles.css` 当前物理行预算。

新增样式必须通过 `scripts/check-style-governance.mjs`。

## 9. 工作区宽度与滚动

根据 active OpenSpec：

- 桌面端 `.app-main` 不因节点内容新增页面级横向或纵向滚动；
- 左侧流程树和右侧节点业务卡片分别承担纵向滚动；
- 节点根元素和普通内容不得超过父业务卡片宽度；
- 固定最小宽度表格必须在局部容器横向滚动；
- 工作区容器不超过760px时，恢复 `.app-main` 页面级纵向滚动，节点卡片自然增高并使用 `overflow: visible`；
- 宽表格在窄屏仍可保留局部横向滚动。

## 10. 业务等价与清理

页面拆分后、视觉迁移前先验证：

- props、emits、API、payload、权限和状态语义不变；
- 保存、提交、审批、退回、重提和文件操作正常；
- 自动推进、工作台待办和当前节点同步；
- URL 刷新仍能打开正确节点；
- API 失败、用户取消和重复点击状态可恢复。

确认无引用后删除旧总面板、空壳迁移组件、失效 import 和重复 CSS。必须使用 `rg` 证明无引用，不得凭文件名直接删除兼容实现。

## 11. 验证门禁

前端每批必须执行：

```powershell
cd digital-platform-web
npm.cmd run check

cd ..
openspec validate --all --strict
git diff --check
```

同时静态检查：

- 节点注册和 fallback 完整；
- 旧总面板、旧控件类和失效 import 无残留；
- 上传表格的 ARIA 结构、按钮文案、权限、pending key 和退回后“整改重传”状态一致；
- 正式审批和客户退回统一使用 `ApprovalActionCard`，审批区位于节点主要业务内容下方；
- 原生表单控件、scoped 样式、`:deep()` 和全局预算符合治理清单；
- 删除文件未被路由、测试、README、docs 或 OpenSpec 引用。

人工视觉回归至少覆盖1280px、768px、375px，以及 loading、empty、error、项目结束、退回、阻塞、readonly、disabled、无文件、长文件名、长文本、大量数据、重复点击、取消确认、生成文件失败、键盘操作和表格溢出。依赖真实业务模板的后端测试必须在真实模板存在时执行，不得使用伪模板把环境缺失声明为通过。

## 12. 完成定义

项目阶段前端交付同时满足以下条件才算完成：

1. 节点通过统一注册体系进入项目工作区，不依赖包含全阶段分支的总面板。
2. 共享页面只承载业务语义一致的稳定能力，独立资料和节点上下文未被合并。
3. API、DTO、权限、状态机、revision、刷新协议和自动推进保持后端权威。
4. 在线表单具备提交校验、错误提示、数据保留和生成状态反馈。
5. 标准控件使用 Element Plus，样式职责符合自动治理门禁。
6. 桌面、窄屏和宽表格滚动行为符合 active OpenSpec。
7. 前端检查、OpenSpec strict validate、diff check 和关键业务回归通过。
8. 未把未执行的人工视觉检查声明为通过。
9. 上传按钮、下载按钮、退回重传文案及审批卡片位置在各阶段遵循统一交互规则。
