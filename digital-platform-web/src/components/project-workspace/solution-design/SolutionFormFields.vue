<template><el-form label-position="top" class="solution-form-grid"><el-form-item v-for="field in fields"
            ...
            :class="{ 'solution-form-grid__wide': field.type === 'textarea' }"><el-date-picker
                v-if="field.type === 'date'" :model-value="displayValue(model[field.key])" type="date"
                value-format="YYYY-MM-DD" :disabled="disabled" @update:model-value="update(field, $event)" /><el-input
                v-else :model-value="displayValue(model[field.key])"
