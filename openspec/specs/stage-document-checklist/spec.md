...
阶段资料能力 MUST 为资料级审核和阶段推进提供资料完成依据，但 MUST NOT 在资料附件、资料提交或资料责任人能力中自动驱动资料审核；配置的写操作完成后 MAY 调用统一阶段门禁，且只有门禁满足时才允许自动推进。当前流程无泛化阶段审批前置。
...
#### Scenario: 标记不适用后调用统一门禁
- **WHEN** 有权用户标记资料不适用后当前阶段齐套摘要变为完成
- **THEN** 系统 MUST NOT 因适用性变化绕过阶段门禁
- **AND** 若该适用性写操作被配置为自动推进触发点，系统 MUST 调用统一阶段门禁并在满足时自动推进
...
#### Scenario: 资料完成后调用统一门禁
- **WHEN** 资料项提交、确认或按 `completionMode` 派生为完成
- **THEN** 系统 MUST NOT 因资料完成绕过当前阶段门禁、返工门禁或标准阶段顺序
- **AND** 若该资料写操作被配置为自动推进触发点，系统 MUST 调用统一阶段门禁并在满足时自动推进
