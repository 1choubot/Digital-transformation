...
上传型节点优先使用统一的表头、数据行、文件信息和操作列结构。审批、退回等正式节点动作不得嵌入上传表格行，应使用统一审批组件放在节点主要业务内容或上传表格下方，与方案设计节点的动作层级一致。简单状态页应使用标题、状态、描述信息和动作的平铺层级，避免在项目工作区节点卡片内继续嵌套无必要的卡片容器。
...
- 表格至少表达上传内容、文件信息和操作。是否展示额外业务列必须由实际信息价值决定；合同上传表格不展示独立“状态”列；
- 文件信息保留文件名及业务需要的大小、上传人和上传时间；长文件名省略显示并通过 `title` 保留完整内容；
