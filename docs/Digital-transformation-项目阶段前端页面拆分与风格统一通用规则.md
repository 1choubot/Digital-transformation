...
合同签订阶段的准备、签订和预付款节点均已通过当前节点页面体系注册。三个节点共享合同阶段布局、业务分区和上传组件，但继续保留独立页面入口、权限、文件槽、付款流程和节点动作。
...
上传型节点优先使用统一的表头、数据行、文件信息和操作列结构；审批、退回等节点专属动作可以作为对应数据行的扩展区。简单状态页应使用标题、状态、描述信息和动作的平铺层级，避免在项目工作区节点卡片内继续嵌套无必要的卡片容器。
...
审批、退回和结束项目类交互优先使用统一 `ApprovalActionCard`，不得在节点页面重复实现意见必填、pending 锁定和按钮禁用规则。需要先选择业务分支的审批通过 selection 插槽表达，并由组件统一锁定未完成选择时的审批按钮；危险操作的二次确认仍由业务 composable 负责。
