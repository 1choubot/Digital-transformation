...
- **AND** 业务展示、生成文件、资料清单和预付款节点结果 MUST 使用 `项目启动通知`
- **AND** 合同 workflow 的项目预付款最终动作生成 C25 后 MUST 能完成 C25 资料完成结果
- **AND** 系统 MUST NOT 新增资料项
- **AND** 若当前运行资料行仍使用旧稳定编码 `4.1`，系统 MUST 将其识别为 C25 对应资料并由合同 workflow 派生完成

#### Scenario: 生成文件不改变资料数量
- **WHEN** 系统生成项目启动通知文件
- **THEN** 生成文件 MUST 作为 C25 的产出查看形态
...
- **THEN** C25 MUST 由合同 workflow 项目预付款支付节点的生成结果承载
