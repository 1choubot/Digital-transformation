<template>
  <section class="project-workspace__detail project-notice-node-page">
    ...
    <section v-else-if="noticeOutput?.formAvailable" class="state-panel state-panel--inline">
...
const NOTICE_DOCUMENT_CODE = '1.3';
const emptyObject = Object.freeze({});
const requestedDocumentId = ref(null);

const context = computed(() => props.nodePageContext || emptyObject);
const noticeOutput = computed(() =>
  (props.node?.outputs || []).find(
    (item) => String(item.documentCode || item.legacyDocumentCode || '') === NOTICE_DOCUMENT_CODE
  ) || null
);
const isActiveOutputForm = computed(
  () =>
    noticeOutput.value?.documentId &&
    String(context.value.activeOnlineFormDocumentId || '') === String(noticeOutput.value.documentId)
);
const activeForm = computed(() =>
  isActiveOutputForm.value ? context.value.activeOnlineForm || null : null
);
const unavailableMessage = computed(() => {
  if (!noticeOutput.value) {
    return `当前节点尚未返回 ${NOTICE_DOCUMENT_CODE} 在线表单。`;
  }
...
watch(
  () => noticeOutput.value?.documentId || null,
  (documentId) => {
...
function notifyFormChanged() {
  emit('business-state-changed', {
    changedDocumentIds: noticeOutput.value?.documentId ? [noticeOutput.value.documentId] : [],
