<template>
  <section v-if="review" class="initiation-review-panel" aria-label="1.2 项目立项多节点审批">
    ...
    <div class="initiation-review-nodes">
      <article v-for="node in nodes" :key="node.nodeKey" class="initiation-review-node">
        ...
        <div v-if="node.canAct" class="initiation-review-actions">
          ...
          >
            {{ isNodePending(node, 'return') ? '退回中...' : '退回' }}
          </button>
        </div>
      </article>
    </div>
...
function formatNodeStatus(status) {
  return {
    ...
    pending: '待审批',
    approved: '已通过',
    returned_blocked_by_rework: '已退回，返工阻塞',
    waiting_prerequisite: '等待前置审批',
