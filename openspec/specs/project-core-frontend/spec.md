...
- **THEN** 前端 MUST 展示基础信息、商务板块、技术板块和三方意见展示区域
- **AND** 前端 MUST 展示商务 7 项评分和技术 4 项评分
- **AND** 每项评分 MUST 在同一横向行中展示评价项、评价标准、分值 0-5、信息收集说明、责任人
- **AND** 窄屏时评分区 MUST 允许横向滚动，不得将同一评分项拆成纵向卡片
- **AND** 每项评分 MUST 展示 schema 预置的真实模板条款内容和评价标准文本
- **AND** 每项评分 MUST 保留信息收集说明列；模板行内说明为空时也 MUST 显示可填写或待补充的说明字段
- **AND** 前端 MUST 允许商务负责人填写整张表，包括技术板块评分
- **AND** 前端 MUST NOT 为技术负责人新增表单填写任务

#### Scenario: 渲染 1.2 真实评分文本
- **WHEN** 商务负责人查看 `1.2 项目立项审批表` 评分区
- **THEN** 前端 MUST 展示商务板块 `甲方属性`、`甲方企业信息`、`身份角色`、`公司竞争优势`、`商务形式及背调`、`商务关系层级`、`项目情况` 的模板条款内容和评价标准
...
- **THEN** 前端 MUST 允许填写新版商务板块
- **AND** 前端 MUST 默认展开商务板块并收起技术板块
- **AND** 前端 MUST NOT 要求商务负责人填写项目开展模式单选字段
- **AND** 前端 MUST NOT 提供项目编号填写或确认入口

#### Scenario: 技术负责人填写新版技术区域
- **WHEN** 技术负责人打开 `1.2 项目立项审批表`
- **THEN** 前端 MUST 允许填写新版技术板块
- **AND** 前端 MUST 默认展开技术板块并收起商务板块
- **AND** 前端 MUST NOT 允许技术负责人修改商务板块

#### Scenario: 其他用户查看新版评分板块
- **WHEN** 无明确商务或技术填写部分的用户打开 `1.2 项目立项审批表`
- **THEN** 前端 MUST 默认展开商务板块和技术板块
- **AND** 用户 MUST 能独立展开或收起任一板块
- **AND** 提交校验失败时前端 MUST 自动展开首个缺失必填字段所在板块并定位该字段

#### Scenario: 不展示审批记录卡片
- **WHEN** 用户打开 `1.2 项目立项审批表`
- **THEN** 前端 MUST NOT 展示营销评价、研发评价和总经理审批的只读记录卡片
- **AND** 前端 MUST 继续向有权用户展示评价、退回和总经理最终审批操作
...
#### Scenario: 1.2 评分板块默认展开范围
- **WHEN** 商务负责人或技术负责人打开 `1.2`
- **THEN** 前端 MUST 默认只展开当前用户可编辑的评分板块
- **AND** 前端 MUST 默认折叠其他评分板块
- **WHEN** 总经理或只读用户打开 `1.2`
- **THEN** 前端 MUST 默认折叠商务和技术评分板块
- **AND** 用户 MUST 仍可手动展开评分板块
- **AND** 提交校验发现必填评分缺失时前端 MUST 自动展开并定位对应板块

#### Scenario: 1.2 五列评分表局部滚动
- **WHEN** 用户查看 `1.2` 商务或技术评分板块
- **THEN** 前端 MUST 保持评价项、评价标准、分值、信息收集说明和责任人五列结构
- **AND** 可用宽度不足时 MUST 仅在评分表容器内提供横向滚动
- **AND** 评分表和审批操作区 MUST NOT 撑宽项目工作区或产生整页横向滚动
...
### Requirement: 项目节点使用统一局部滚动边界
项目核心前端 MUST 在桌面端将项目工作区限制在主布局可用高度内，并由左侧流程树和右侧节点业务卡片分别承担纵向滚动；节点内宽表格 MUST 使用边界明确的局部横向滚动容器。

#### Scenario: 桌面端节点内容独立纵向滚动
- **WHEN** 用户在桌面端查看内容超过工作区高度的项目节点页
- **THEN** 主布局 `.app-main` MUST NOT 因节点内容展开而新增横向或纵向滚动
- **AND** 右侧节点业务卡片 MUST 在自身内容区提供纵向滚动
- **AND** 左侧流程树 MUST 保持独立纵向滚动
...
- **WHEN** 项目工作区通过节点路由展示任意现有或后续新增节点页面
- **THEN** 节点页面根元素 MUST 使用父业务卡片的可用宽度
- **AND** 节点页面及其普通内容 MUST NOT 超过父业务卡片宽度
...
#### Scenario: 成本估算上传表仅展示紧邻上一节点文件
- **WHEN** 前端展示制造成本估算、营销成本估算或财务成本估算节点的上传表
- **THEN** 制造成本估算上传表 MUST 仅展示研发中心成本估算表和当前制造中心成本估算表上传行
- **AND** 营销成本估算上传表 MUST 仅展示制造中心成本估算表和当前营销中心成本估算表上传行
- **AND** 财务成本估算上传表 MUST 仅展示营销中心成本估算表和当前财务/运营成本估算表上传行
- **AND** 紧邻上一节点的文件行 MUST 只允许下载，不得允许上传、替换或修改豁免状态
